...
- [x] 修复数据库缺失 `is_deleted` 字段导致任务列表无法显示的问题 (Added explicit migration) ✅ 2026-02-11 06:10

### 2026-02-11 06:45 - 核心功能修复 & 系统稳定性增强 ✅

- [x] 修复任务详情页面 (TaskDetail.vue) 渲染崩溃的问题，解决由于数据未就绪或模板逻辑错误导致的页面白屏。
- [x] 优化 `TaskDetail.vue` 组件逻辑，增加数据加载状态监控与错误兜底，提升页面健壮性。
- [x] 恢复任务详情核心功能（状态流转、进度更新、系数调整、子任务展示），解决点击任务进入显示为空的问题。

### 2026-02-11 23:40 - Git 同步策略优化与冗余清理 ✅

- [x] 清理 `PMS` 子项目中的开发环境与编译缓存：
  - [x] 在 `PMS` 目录下创建并完善 `.gitignore`，排除 `venv/` 和 `__pycache__`。
  - [x] 从 Git 索引中移除已追踪的冗余二进制文件（保留本地文件）。
  - [x] 统一父仓库与子项目的过滤规则，减少 700+ 无用文件的同步干扰。
