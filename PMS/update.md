...
- [x] 任务不能删除 (后端已限制，前端需配合)
- [ ] 每个任务应挂钩「责任人」 ❌
- [ ] 附件保存到数据库
- [ ] 可以设置上传附件类型、附件的的大小 (后端配置已生效)
- [x] 尝试添加一个新部门。（编辑部门无法保存、新增公司报错、新增部门报错）✅ 2026-02-10
- [x] 尝试删除一个无人的空部门。✅ 2026-02-10
- [x] 岗位库可以编辑 ✅ 2026-02-10
- [ ] 角色增加一个组长，组长管理员工
- [x] 任务需增加「待提交」✅ 2026-02-10 (Pending Submission)
- [ ] 过程中可以修改重要性和难度
- [x] 任务的审批权限有问题，与选择不对应 ✅ 2026-02-10 (增加 reviewer_id 强校验)
- [ ] 完善权限设置

### 2026-02-10 17:16 - 第一阶段：夯实底座完成 ✅

- [x] **组织架构管理修复**:
  - [x] 重写后端 `org.py`，修复事务未提交导致的新增/编辑失败问题
  - [x] 补全 `get_org_tree` 返回字段，前端 OrgManager 实现完整的公司/部门/岗位 CRUD
  - [x] 岗位库编辑：支持新增、编辑名称/权限、删除（带关联检查）
- [x] **审批权限纠正**:
  - [x] 后端 `approve`/`reject`/`return`/`review` 接口增加 `reviewer_id` 强校验，防止非指定审批人越权操作
  - [x] 前端 `TaskDetail` 按钮显隐逻辑优化，仅对相关责任人显示操作按钮
- [x] **待提交状态 (Pending Submission)**:
  - [x] 任务模型新增 `PENDING_SUBMISSION` 状态
  - [x] 任务表单新增「标记待提交」功能，允许在正式提交前暂存为待提交状态
  - [x] 支持从待提交状态进行编辑和正式提交
