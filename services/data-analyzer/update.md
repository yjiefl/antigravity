...
- [x] 修复获取历史辐照度时的报错（需排查后端日志） (2026-02-08 已完成)
- [x] 获取时自动匹配现有分组维度（如“场站”/“Station”），避免重复分组 (2026-02-08 已完成)

#### Task 2: 限电逻辑精细化

- [ ] 实现点对点限电判断逻辑，确保导出结果与图表展示一致

#### Task 3: UI/UX 深度联动

- [ ] 实现「坐标轴设置」与图例显示的强联动（隐藏系列时置灰/隐藏坐标轴）
- [ ] 优化默认曲线颜色方案，使用不同的默认颜色，提升多曲线辨识度
