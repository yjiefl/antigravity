...
确保 UI 重构不破坏现有的 CSV 解析逻辑、历史快照读取、等所有已有功能。
极简主义 (Minimalism)：消除一切不必要的空间浪费，移除冗余的装饰。
高信息密度 (High Density)：采用较小的字号和紧凑的布局，确保在不滚动的情况下展示更多有效分析维度。
PC 优先 (PC-Only)：完全放弃移动端适配，专注于大屏显示器的操作效率。

具体优化任务：

1. 布局与空间优化 (Layout & Space)
最大化图表区：将 ECharts 容器设置为动态响应，确保其占据屏幕 85% 以上的视觉重心。
超紧凑侧边栏：
进一步压缩侧边栏宽度，字号统一调整为 11px 或 10px。
所有的控制项（开关、滑块、输入框）采用行内布局（Inline），减少垂直堆叠。
侧边栏背景使用更通透的 backdrop-filter: blur(20px)，边框改为 0.5px 的半透明线条。
2. 视觉与色彩 (Visuals & Colors)
字体系统：全局使用 Poppins (标题/数字) 和 Open Sans (正文)。数字显示采用等宽字体，确保在对比时整齐划一。
支持深色模式/浅色模式
曲线色板：引入一套 20 色的高对比度专业色板，确保多场站、多指标叠加时辨识度极高。
3. 交互细节增强 (Interaction)
联动置灰效果：当图例被隐藏时，侧边栏对应的配置项不仅要置灰，还要应用 saturate(0) 滤镜，使视觉焦点始终集中在活跃曲线上。
Tooltip 优化：提示框背景改为全透明磨砂质感，文字排版更加紧凑，关键数值加粗显示。
4. 性能与规范
确保所有的 UI 修改不影响 ECharts 的渲染性能。
修改完成后，请更新 services/data-analyzer/update.md，记录此次 UI/UX 深度优化。
