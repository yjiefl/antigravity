...
8. 改进：上部图例超出则分行，不显示「粘贴数据\_2033」等文件名相关字眼，仅在左侧显示 2026年1月29日 21:32 ✅已完成
...
157. **2026-02-08**:
    ...
    - [x] UI/UX 深度重构: 全面升级为高密度、极简主义设计，针对 PC 端大屏优化布局。 (修改人: Antigravity) ✅ 2026年2月8日 20:05
    - [x] 维度筛选逻辑修复: 修复全局全选失效 Bug，统一“全选/清空/反选”三图标交互模式。 (修改人: Antigravity) ✅ 2026年2月8日 21:25

158. **2026-02-09**:


    - **BUG修复**: 修复历史辐照度颜色调整失效问题。 (修改人: Antigravity) ✅2026-02-09 11:55
    - **BUG修复**: 修复数据开放显示与坐标轴隐藏不对应问题。通过统一指标标识符（Canonical Metric Key）逻辑，确保侧边栏设置、图例控制与图表渲染在标识符层面的完全一致性。 (修改人: Antigravity) ✅2026-02-09 11:55
    - **UI优化**: 将全局缩放滑动条宽度限制为80px，彻底解决百分比显示被遮挡的问题。 (修改人: Antigravity) ✅2026-02-09 12:47
    - **UI优化**: 优化自动配色逻辑，引入优先级机制，为“历史辐照度”、“出清曲线”、“主站可用”等指标分配独立的固定颜色，解决与常规指标颜色撞车的问题。 (修改人: Antigravity) ✅2026-02-09 12:50

159. **2026-02-09 (持续)**:


    - **新功能**: 在图表提示框 (Tooltip) 中增加「限电状态」标识，通过红色/绿色直观区分当前点是否处于限电状态。 (修改人: Antigravity) ✅2026-02-09 13:25
    - **BUG修复**: 修复提示框限电判定边界 Bug。通过引入区间闭合/半开判定（Exclusive End）与精确点对点状态校验，彻底解决了色带边缘误报的问题。 (修改人: Antigravity) ✅2026-02-09 13:42
    - **UI优化**: 修复浅色模式下侧边栏数据数值看不清的问题。移除了容器透明度限制，并将数值颜色调整为高对比度深蓝，同时优化了按钮的悬停显示。 (修改人: Antigravity) ✅2026-02-09 13:42
    - **运维优化**: 优化 VPS 部署脚本 (`deploy_to_vps.sh`)。增加了生产环境数据库 (`data.db`) 与日志目录的排除保护，防止本地测试数据意外覆盖远程生产数据。 (修改人: Antigravity) ✅2026-02-09 13:45

203: - **BUG修复**: 修复 CSV 导出数据不全、指标相互覆盖的严重 Bug。原逻辑由于在原始 state 中查找不存在的 `metricKey` 导致多个指标被映射到 `undefined` 并互相覆盖。现统一使用 `activeSeries` 导出，确保导出数据与图表显示完全一致。 (修改人: Antigravity) ✅2026-02-09 16:25
204: - **运维优化**: 新增 `update_vps_db.sh` 脚本，支持一键将本地数据库强制同步至 VPS，并自动修复远程 `backend/` 目录的读写权限。 (修改人: Antigravity) ✅2026-02-09 16:30
...
#### UI改进 (2026-02-08 全部完成 ✅)

1. 维度筛选「全选」「清空」「反选」按钮图标化，间距优化 ✅
2. 「场站名称」高对比度行业色板 ✅
3. 「限电分析」输入框高亮底色与边框 ✅
4. 「限电分析」滑块增长，颜色选择区域优化 ✅
5. 「导出CSV」「重置」「清空」按钮图标化 (圆形背景) ✅
6. 「坐标轴与单位设置」空间压缩与高密度布局 ✅

### bug (已修复 ✅)

2026年2月9日 12:47 导出CSV文件已恢复 ✅
2026年2月9日 12:47 清空按钮已恢复并重构为图标 ✅

#### UI改进 (2026年2月9日 12:54 ✅)

颜色分配逻辑重构：弃用了简单的对象映射，改为使用有序数组进行颜色匹配。
优先级机制：将“历史辐照度”、“A3主站”、“短波辐射”等具体指标放在匹配列表的前列，确保它们不会被更通用的“辐照度”或“可用”等规则误匹配。
新配色方案：
历史辐照度：分配了 紫色 (#a855f7)，以区别于实时的金黄色辐照度。
出清曲线：分配了 玫红 (#db2777)。
A3主站/主站可用：分配了 青色 (#06b6d4)。
短波辐射：分配了 深橙色 (#f97316)。

#### 限电逻辑精细化

- **光伏模式支持**: 新增“光伏”快捷选项，勾选后自动填入 07:00-19:00 区间，并支持后续自定义微调，优化光伏分析场景下的 X 轴缩放体验。 (2026-02-09 16:48:00)
- **场站库导出支持**: 场站库管理新增 CSV 批量导出按钮，支持导出场站名称、经纬度、方位角、倾角及区域等全量配置，方便离线编辑与数据迁移。 (2026-02-09 17:15:30)
- **导入覆盖修复**: 修复了场站库导入时由于名称空格导致无法覆盖同名场站的问题，同时加强了后端写入的事务处理，确保数据一致性。 (2026-02-09 18:00:00)
