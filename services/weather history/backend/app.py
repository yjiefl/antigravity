...
def create_app():
    ...
    def before_request():
        ...
        # API 频率限制 (Item 84/后续改进)
        if request.path.startswith('/api') and request.path != '/api/health':
            ip = request.remote_addr
            is_allowed, remaining = rate_limiter.is_allowed(ip)
            if not is_allowed:
                return jsonify({
                    'code': 429,
    ...
    def after_request(response):
        if request.path.startswith('/api') and request.path != '/api/health':
            try:
                ...
                host_name = None
                try:
                    # 对于本地访问 (127.0.0.1)，socket.gethostname() 更准确
                    if ip == '127.0.0.1':
                        host_name = socket.gethostname()
                    else:
                        # 尝试反向 DNS (可能会慢，生产环境建议异步处理)
                        host_name = socket.gethostbyaddr(ip)[0]
                except Exception:
                    host_name = "Unknown"

                db_manager.log_request(ip, ua, request.path, request.method, response.status_code, latency, host_name)
