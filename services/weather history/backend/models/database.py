...
class DatabaseManager:
    ...
    def init_database(self):
        ...
        try:
            ...
            try:
                cursor.execute('ALTER TABLE system_logs ADD COLUMN host_name TEXT')
                logger.info("添加 system_logs.host_name 列成功")
            except sqlite3.OperationalError:
                pass
    ...
    def log_request(self, ip: str, ua: str, path: str, method: str, status: int, latency: float, host_name: str = None):
        ...
        try:
            sql = '''
                INSERT INTO system_logs (ip_address, host_name, user_agent, path, method, status_code, latency_ms)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            '''
            self.execute_update(sql, (ip, host_name, ua, path, method, status, latency))
        except Exception as e:
            # 日志记录失败不应该影响主流程，仅打印错误
            logger.error(f"记录请求日志失败: {e}")

    def log_event(self, level: str, source: str, message: str, details: str = None):
        """
        记录事件日志 (系统告警等)
        """
        try:
            sql = '''
                INSERT INTO event_logs (level, source, message, details)
                VALUES (?, ?, ?, ?)
            '''
            self.execute_update(sql, (level, source, message, details))
        except Exception as e:
    ...
    def get_event_logs(self, limit: int = 100) -> List[Dict[str, Any]]:
        """
        获取事件日志列表
        """
        sql = "SELECT * FROM event_logs ORDER BY created_at DESC LIMIT ?"
        return self.execute_query(sql, (limit,))
