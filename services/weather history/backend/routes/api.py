...
@api_bp.route('/weather/forecast-query', methods=['POST'])
def query_forecast():
    """
    查询预测天气数据（与历史数据查询格式一致）
    ...
    Returns:
        JSON响应，包含15分钟精度数据、7天预报和统计信息
    """
    try:
        data = request.get_json()
        city_ids = data.get('city_ids', [])
        # 兼容旧的单城市参数
        if not city_ids and data.get('city_id'):
            city_ids = [data.get('city_id')]
        ...
        for city_id in city_ids:
            ...
            # 获取预测数据
            forecast_data = weather_service.get_forecast(city_id, days)
            
            # 获取实时数据
            try:
                current_weather = weather_service.get_current_weather(city_id)
                # 补充坐标信息用于前端展示
                if current_weather:
                    current_weather['longitude'] = city_info['longitude']
                    current_weather['latitude'] = city_info['latitude']
            except Exception as e:
                logger.warning(f"获取实时天气失败 (City ID: {city_id}): {e}")
                current_weather = None
            
            # 转换 minutely_15_forecast 为 records 格式（与历史数据查询一致）
            minutely_data = forecast_data.get('minutely_15_forecast', [])
            hourly_data = forecast_data.get('hourly_forecast', [])
            daily_data = forecast_data.get('daily_forecast', [])
            
            # 使用15分钟数据作为主要记录，如果没有则使用小时数据
            if minutely_data:
                records = [
                    {
                        'datetime': item['time'],
                        'temperature_2m': item.get('temp'),
                        'wind_speed_10m': item.get('wind'),
                        'shortwave_radiation': item.get('radiation'),
                        'precipitation_probability': item.get('pop'),
                        'precipitation': item.get('rain', 0),
                        'weather_code': item.get('weather_code', 0)
                    }
                ...
                records = [
                    {
                        'datetime': item['time'],
                        'temperature_2m': item.get('temp'),
                        'relative_humidity_2m': item.get('humidity'),
                        'wind_speed_10m': item.get('wind'),
                        'shortwave_radiation': item.get('radiation'),
                        'precipitation_probability': item.get('pop'),
                        'precipitation': item.get('rain', 0),
            ...
            # 计算统计摘要
            summary = data_analyzer.calculate_summary(records)
            
            results.append({
                'city_id': city_id,
                'city_name': forecast_data.get('city_name', city_info['city_name']),
                ...
                'data_precision': '15min' if minutely_data else 'hourly'
            })
...
@api_bp.route('/forecast/archive', methods=['POST'])
def archive_forecast_endpoint():
    """
    存档当前预测数据
    """
    try:
        ...
        if not city_id:
            return jsonify({
                'code': 400,
                'message': '缺少参数: city_id',
                'data': None
            }), 400
            
        result = weather_service.archive_forecast(city_id)
...
def get_forecast_history_list():
    ...
    try:
        ...
        # Access DB via weather_service
        history = weather_service.db_manager.get_forecast_history(city_id, limit)
...
@api_bp.route('/forecast/history/<int:snapshot_id>', methods=['GET'])
def get_forecast_snapshot_detail(snapshot_id):
    """
    获取单条预测快照详情
    """
    try:
        snapshot = weather_service.db_manager.get_forecast_snapshot(snapshot_id)
...
def get_system_logs():
    ...
    try:
        ...
        # Access DB via weather_service.db_manager which is initialized globally
        logs = weather_service.db_manager.get_system_logs(limit)
...
def get_system_events():
    ...
    try:
        limit = request.args.get('limit', 100, type=int)
        logs = weather_service.db_manager.get_event_logs(limit)
...
def get_settings():
    """获取系统设置"""
    try:
        settings = weather_service.db_manager.execute_query("SELECT key, value FROM system_settings")
        # Convert list of dicts to a single dict
        settings_dict = {s['key']: s['value'] for s in settings}
        return jsonify({'code': 200, 'data': settings_dict})
    except Exception as e:
...
def update_settings():
    ...
    try:
        ...
        for key, value in data.items():
            weather_service.db_manager.execute_update(
                "INSERT OR REPLACE INTO system_settings (key, value, updated_at) VALUES (?, ?, CURRENT_TIMESTAMP)",
                (key, str(value))
            )
        return jsonify({'code': 200, 'message': '设置更新成功'})
    except Exception as e:
        return jsonify({'code': 500, 'message': str(e)}), 500

@api_bp.route('/system/events/test', methods=['POST'])
def create_test_event():
    """创建测试告警事件"""
    try:
        weather_service.db_manager.log_event("WARNING", "TEST_UI", "这是一条测试告警消息", "用户在后台点击了发送测试告警按钮")
        return jsonify({'code': 200, 'message': '已发送测试告警'})
    except Exception as e:
        return jsonify({'code': 500, 'message': str(e)}), 500
