...
async function initDataManagement() {
    ...
    const bindings = {
        ...
        'addCustomCityBtn': handleCustomCityAdd,
        'archiveForecastBtn': handleArchiveForecast,
        'refreshHistoryBtn': () => {
             const cityId = getSelectedHistoryCityId();
             loadForecastHistory(cityId);
        }
    };

    // 为复盘页渲染城市列表
    renderHistoryCitySelector();
    ...
    loadDataStatistics();
    loadManagedCities();
    initSystemLogs(); // System Logs Init
    initSystemAlerts(); // System Alerts & Settings Init
    populateForecastHistoryCities(); // Populate cities for forecast history
    loadForecastHistory(); // Load initial history list
...
// --- Forecast History Logic ---

function populateForecastHistoryCities() {
    // We use a radio group behavior for forecast history to select ONE city
    const container = document.getElementById('historyCitySelect');
    if (!container) return;
    
    container.innerHTML = '';
    appState.cities.forEach(city => {
        const div = document.createElement('div');
        div.className = 'field-checkbox'; // Reuse style
        div.innerHTML = `
            <input type="radio" name="history-city" id="hist_city_${city.id}" value="${city.id}">
            <label for="hist_city_${city.id}">${city.name}</label>
        `;
        div.querySelector('input').addEventListener('change', (e) => {
            if (e.target.checked) {
                loadForecastHistory(e.target.value);
            }
        });
...
function getSelectedHistoryCityId() {
    const checked = document.querySelector('input[name="history-city"]:checked');
    return checked ? checked.value : null;
}

async function handleArchiveForecast() {
    const cityId = getSelectedHistoryCityId();
    if (!cityId) {
        alert('请先选择一个城市');
        return;
    ...
    const city = appState.cities.find(c => String(c.id) === String(cityId));
    if (!confirm(`确定要为 ${city.name} 存档当前的预测数据吗？`)) return;
    
    console.log(`Starting archival for city: ${city.name} (ID: ${cityId})`);
    const btn = document.getElementById('archiveForecastBtn');
    const originalContent = btn.innerHTML;
    btn.disabled = true;
    btn.innerHTML = '存档中...';
    
    try {
        const res = await api.archiveForecast(cityId);
        console.log('Archival response:', res);
        alert('存档成功');
        loadForecastHistory(cityId); // Refresh list
    } catch (e) {
        ...
        console.log('Archival process finished.');
    }
}
...
async function loadForecastHistory() {
    ...
    try {
        const res = await api.getForecastHistory(cityId, 50); // Get last 50
        const list = res.data;
        
        if (!list || list.length === 0) {
            tbody.innerHTML = '<tr><td colspan="3" style="text-align:center;color:var(--text-muted)">暂无存档记录</td></tr>';
            return;
        }
        
        tbody.innerHTML = '';
        list.forEach(item => {
            const city = appState.cities.find(c => c.id === item.city_id);
            const cityName = city ? city.name : item.city_id;
            ...
            tr.innerHTML = `
                ...
                <td style="display:flex; gap:8px;">
                    <button class="btn btn-tiny btn-secondary" onclick="viewHistorySnapshot(${item.id})">查看趋势</button>
                    <button class="btn btn-tiny btn-danger" onclick="deleteHistorySnapshot(${item.id}, ${cityId})">删除</button>
                </td>
            `;
...
async function deleteHistorySnapshot(id, cityId) {
    if (!confirm('确定要删除这条存档记录吗？')) return;
    try {
        await api.deleteForecastSnapshot(id);
        loadForecastHistory(cityId);
    } catch (e) {
...
async function viewHistorySnapshot(snapshotId) {
    try {
        const res = await api.getForecastSnapshot(snapshotId);
        const snapshot = res.data;
        
        if (!snapshot) throw new Error('未找到数据');
        
        const rawData = JSON.parse(snapshot.raw_data);
        
        // 获取城市名称
        let cityName = `城市 ID:${snapshot.city_id}`;
        if (appState.cities && appState.cities.length > 0) {
            const city = appState.cities.find(c => String(c.id) === String(snapshot.city_id));
            if (city) cityName = city.name;
        }
        
        if (typeof window.renderReviewDashboard === 'function') {
            window.renderReviewDashboard(rawData, cityName, snapshot.created_at);
        } else {
            console.error('renderReviewDashboard function not found');
            alert('无法展示复盘看板: 缺少显示组件');
        }
...
window.deleteHistorySnapshot = deleteHistorySnapshot;
window.loadForecastHistory = loadForecastHistory;
window.handleArchiveForecast = handleArchiveForecast;
...
function initSystemLogs() {
    ...
    if (refreshBtn) {
        ...
        // Auto load when tab is switched to system-logs
        const tabBtn = document.querySelector('.tab-btn[data-tab="system-logs"]');
        if (tabBtn) {
             tabBtn.addEventListener('click', () => {
                 // only load if table is empty or explicitly requested?
                 // Simple approach: load every time tab is clicked
                 loadSystemLogs();
             });
        }
    }
...
async function loadSystemLogs() {
    ...
    try {
        ...
        if (!logs || logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;color:var(--text-muted)">暂无日志记录</td></tr>';
            return;
        }
        ...
        logs.forEach(log => {
            ...
            if (log.status_code >= 500) statusColor = 'var(--color-danger)';
            else if (log.status_code >= 400) statusColor = 'orange';
            else if (log.status_code >= 200) statusColor = 'var(--color-success)';
            
            tr.innerHTML = `
                <td style="font-size:0.85em;color:var(--text-secondary)">${log.created_at}</td>
                <td style="font-weight:bold">${log.method}</td>
                <td style="font-family:monospace;font-size:0.9em">${log.path}</td>
                <td style="color:${statusColor};font-weight:bold">${log.status_code}</td>
                <td>${log.latency_ms ? log.latency_ms.toFixed(1) : '-'}</td>
                <td style="font-size:0.85em;color:var(--text-muted)">${log.host_name || 'Unknown'}</td>
                <td style="font-size:0.85em;color:var(--text-muted)">${log.ip_address}</td>
            `;
            tbody.appendChild(tr);
        });
        ...
        console.error(e);
        tbody.innerHTML = `<tr><td colspan="7" style="text-align:center;color:red">加载日志失败: ${e.message}</td></tr>`;
    }
}
 ...
 * 处理添加自定义坐标城市
 */
async function handleCustomCityAdd() {
    const nameInput = document.getElementById('customCityName');
    const lonInput = document.getElementById('customCityLon');
    const latInput = document.getElementById('customCityLat');
    ...
    const btn = document.getElementById('addCustomCityBtn');
    const originalText = btn.innerHTML;
    btn.disabled = true;
    btn.textContent = '添加中...';
    
    try {
        // Reuse the existing handleAddCity logic which calls API and refreshes UI
        // We catch here only to reset UI button, handleAddCity handles its own alerts for failures usually?
        // Checking handleAddCity: it alerts on failure but throws error too?
        // "catch (error) { alert('添加失败: ' + error.message); }" inside handleAddCity
        // So we don't need double alert, just UI reset.
        
        await handleAddCity(name, lon, lat, '自定义');
        
        // Clear inputs on success (handleAddCity doesn't clear these inputs)
        nameInput.value = '';
        lonInput.value = '';
        latInput.value = '';
        
    } catch (e) {
         // handleAddCity already alerted
    } finally {
        btn.disabled = false;
        btn.innerHTML = originalText;
    }
...
function initSystemAlerts() {
    ...
    if (refreshBtn) refreshBtn.onclick = loadSystemAlerts;
    
    const testBtn = document.getElementById('testEventBtn');
    if (testBtn) testBtn.onclick = handleTestEvent;
    
    const saveSettingsBtn = document.getElementById('saveRateLimitBtn');
    if (saveSettingsBtn) saveSettingsBtn.onclick = handleSaveRateLimit;
...
async function loadSystemAlerts() {
    ...
    try {
        const res = await api.getSystemEvents(100);
        const logs = res.data;
        
        if (!logs || logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">无事件记录</td></tr>';
            return;
        }
        ...
        logs.forEach(log => {
            ...
            let levelColor = 'var(--text-primary)';
            if (log.level === 'CRITICAL' || log.level === 'ERROR') levelColor = 'var(--color-danger)';
            else if (log.level === 'WARNING') levelColor = 'orange';
            else if (log.level === 'INFO') levelColor = 'var(--color-success)';
            
            tr.innerHTML = `
                <td style="font-size:0.85em;color:var(--text-secondary)">${log.created_at}</td>
                <td><span class="badge" style="background:${levelColor};color:white;padding:2px 6px;border-radius:4px;font-size:0.75rem">${log.level}</span></td>
                <td><code style="font-size:0.85rem">${log.source}</code></td>
                <td>${log.message}</td>
                <td style="font-size:0.85em;color:var(--text-muted)">${log.details || '-'}</td>
            `;
            tbody.appendChild(tr);
        });
        ...
        console.error(e);
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center;color:red">加载事件失败: ${e.message}</td></tr>`;
    }
}
...
async function loadSystemSettings() {
    try {
        ...
        if (settings && settings.api_rate_limit) {
            const input = document.getElementById('apiRateLimitInput');
            if (input) input.value = settings.api_rate_limit;
        }
    } catch (e) {
...
async function handleSaveRateLimit() {
    ...
    try {
        await api.updateSystemSettings({ api_rate_limit: value });
        alert("系统设置更新成功");
    } catch (e) {
