...
function populateManagementCities() {
  // 渲染三个地方的城市选择器
  CommonUtils.renderCityCheckboxes(
    "downloadCitySelect",
    "dl-city-checkbox",
    "dl",
    true,
  ...
  for (const cityId of cityIds) {
    ...
    try {
      ...
      if (response.code !== 200 && response.code !== 201) {
        // 201 is sometimes used for "created"
...
function updateProgress(currentCityName, actionPrefix = "正在下载") {
  ...
  if (statusText)
    statusText.textContent = `${actionPrefix}: ${currentCityName} (${downloadState.completedChunks + 1}/${downloadState.totalChunks})`;
}
...
async function handleCheckCompleteness() {
  ...
  resultDiv.innerHTML =
    '<div style="text-align:center">正在检查数据完整性...</div>';
  ...
  try {
    ...
    for (const cityId of cityIds) {
      ...
      try {
        ...
        const statusClass =
          data.missing_count === 0 ? "status-complete" : "status-missing";
        const statusText =
          data.missing_count === 0
            ? "数据完整"
            : `缺失 ${data.missing_count} 小时`;
...
async function handleExportBulk() {
  ...
  if (progressContainer) {
    ...
    progressContainer.querySelector(".progress-title").textContent =
      "文件准备进度";
  }
...
function renderManagedCities() {
  ...
  if (cities.length === 0) {
    listContainer.innerHTML =
      '<p style="color: var(--text-muted); text-align: center; padding: 20px;">列表为空，请搜索并添加城市</p>';
    return;
  }
...
function renderSearchResults(results) {
  ...
  if (results.length === 0) {
    resultBox.innerHTML =
      '<div style="padding:10px; text-align:center">未找到匹配城市</div>';
    return;
  }
...
async function handleClearAllCities() {
  if (
    !confirm("确定要清空所有管理城市吗？这不会删除历史气象数据，只是清空列表。")
  )
    return;
...
async function handlePreviewDeleteData() {
  ...
  const targetCityIds =
    cityIds.length > 0 ? cityIds : appState.cities.map((c) => c.id);
  ...
  resultBox.style.display = "block";
  resultBox.innerHTML =
    '<div style="text-align:center;">正在分析数据分布...</div>';
...
async function handleDeleteData() {
  ...
  if (cityIds.length === 0) {
    if (
      !confirm(
        "您未选择特定城市，这将尝试删除所有已选定范围的数据（需配合日期）。是否继续？",
      )
    ) {
      return;
    }
...
async function loadSystemLogs() {
  ...
  try {
    ...
    if (!logs || logs.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="7" style="text-align:center;color:var(--text-muted)">暂无日志记录</td></tr>';
      return;
    }
...
async function loadSystemAlerts() {
  ...
  try {
    ...
    if (!logs || logs.length === 0) {
      tbody.innerHTML =
        '<tr><td colspan="5" style="text-align:center;color:var(--text-muted)">无事件记录</td></tr>';
      return;
    }
