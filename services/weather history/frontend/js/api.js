...
class APIClient {
    ...
    /**
     * 查询预测数据（与历史数据查询格式一致）
     * @param {object} params - 查询参数 {city_id, days, fields}
     * @returns {Promise} 预测数据
     */
    async queryForecast(params) {
        return this.post('/weather/forecast-query', params);
    }
    ...
    async getSystemEvents(limit = 100) {
        return this.get(`/system/events?limit=${limit}`);
    }
     ...
     */
    async sendTestEvent() {
        return this.post(`/system/events/test`, {});
    }
}
