...
- 📊 **多维分析**:
  - **预测查询回溯**: 重构后的「天气预测查询」支持返回 15 分钟高精度预测，整合实时实况同屏显示。
...
- [x] 优化 Excel 导出：取消汇总表，增强每日汇总数据 (2026-01-29)
- [x] 支持 Docker 容器化部署 (2026-01-29)
- [x] 添加天气预测功能 (2026-01-31)
- [ ] **坐标管理**: 支持手动输入经纬度坐标并自定义命名
- [ ] 实现用户账户系统
- [ ] 数据分析功能增强（趋势分析、异常检测）

## 🐋 Docker 部署 (QNAP NAS / 服务器)

项目支持通过 Docker 进行快速部署，特别适合部署在 QNAP NAS 的 Container Station 或局域网服务器中。
