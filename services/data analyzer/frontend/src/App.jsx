...
function App() {
  ...
  const [systemVersion] = useState("v2.0.0-20260210");
    ...
    Object.entries(dimensionGroups).forEach(([groupKey, groupSeries]) => {
      ...
      if (irrad && agc && power) {
        ...
        const pushRange = (start, end, exclusiveEnd = false) => {
          let s = start,
            e = end;
          if (isCompareOverlap && selectedDates.length > 1) {
            const mapTime = (ts) => {
  ...
  const updateChart = () => {
    ...
    // 安全守卫：仅在多日期选择时，重叠对比模式才生效；单日模式下强制关闭
    const effectiveOverlap = isCompareOverlap && selectedDates.length > 1;
                  ...
                  {showCurtailmentAnalysis && (
                    ...
                    >
                      <div className="control-row-dense">
                        <span className="label-mini" style={{ flexShrink: 0 }}>
                          外观
                        </span>
                        <div
                          style={{
