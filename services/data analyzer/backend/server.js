      ...
      () => {
        ...
        db.get("SELECT COUNT(*) as count FROM stations", [], (err, row) => {
          ...
          initialStations.forEach((s) => {
            db.run(
              "INSERT OR IGNORE INTO stations (name, lon, lat, region, azimuth, tilt) VALUES (?,?,?,?,?,?)",
              [...s, 0, 0],
            );
          });
        });
...
app.post("/api/stations", (req, res) => {
  ...
  db.serialize(() => {
    ...
    const stmt = db.prepare(
      "INSERT OR REPLACE INTO stations (name, lon, lat, region, azimuth, tilt) VALUES (?, ?, ?, ?, ?, ?)",
    );
    stations.forEach((s) => {
      stmt.run([
        s.name.trim(),
    ...
    db.run("COMMIT", (err) => {
      if (err) {
        ...
        return res
          .status(500)
          .json({ error: "Database transaction failed: " + err.message });
      }
...
app.put("/api/stations/:oldName", async (req, res) => {
  ...
  try {
    // 1. 检查新名称是否已存在
    const existingNew = await dbGetAsync(
    ...
    if (existingNew) {
      return res
        .status(409)
        .json({ error: `名称 "${newName}" 已存在，无法重命名` });
    }

    // 2. 检查旧场站是否存在
    const station = await dbGetAsync("SELECT * FROM stations WHERE name = ?", [
      oldName,
    ]);
    if (!station) {
      return res
        .status(404)
        .json({ error: `未找到要重命名的场站 "${oldName}"` });
    }
    ...
    try {
      ...
      res.json({
        success: true,
        message: `场站已从 ${oldName} 重命名为 ${newName}`,
      });
...
app.get("/api/weather/irradiance", async (req, res) => {
  ...
  db.get(
    ...
    async (err, coords) => {
      if (err) {
        logToError(
          `Database error fetching station ${stationName}: ${err.message}`,
        );
      ...
      if (!coords) {
        ...
        return res
          .status(404)
          .json({
            error: `未找到场站 "${stationName}" 的坐标映射，请先在场站管理中配置`,
          });
      }
      ...
      if (!coords.lat || !coords.lon) {
        logAction(
          req,
          "Weather Data Failed",
          `Coords missing for: ${stationName}`,
        );
        return res
          .status(400)
          .json({ error: `场站 "${stationName}" 缺少有效的经纬度信息` });
      }

      logAction(
        req,
        "Fetch Weather Data",
        `Station: ${stationName} (${coords.lat}, ${coords.lon}), Date: ${date}`,
      );
      try {
        ...
        if (!response.ok) {
          ...
          return res
            ...
            .json({ error: `天气服务请求失败 (HTTP ${response.status})` });
        }

        const data = await response.json();
        if (!data || !data.hourly) {
          logToError(
            `Open-Meteo returned invalid data format: ${JSON.stringify(data)}`,
          );
          return res
            .status(502)
            .json({ error: "天气服务返回了无效的数据格式" });
        }
        ...
        for (let i = 0; i < times.length; i++) {
          ...
          // Open-Meteo 的辐照度数据是 "preceding hour mean" (前一小时平均值)，即 12:00 代表 11:00-12:00 的数据
          // 为了符合通常的 "start of hour" (小时开始) 展示习惯，我们需要将时间前推 1 小时 (即变为 11:00)
          ...
          for (let j = 0; j < 4; j++) {
            const interpolatedTime = new Date(
              currentTime.getTime() + j * 15 * 60000,
            );
            
            // 确保只保留当天的数据，过滤掉因时间平移而产生的“前一天”或“后一天”的数据
            const year = interpolatedTime.getFullYear();
            const month = String(interpolatedTime.getMonth() + 1).padStart(2, '0');
            const day = String(interpolatedTime.getDate()).padStart(2, '0');
            const currentDateStr = `${year}-${month}-${day}`;
            
            if (currentDateStr !== date) continue;

            const weight = j / 4;
            const interpolatedVal =
              currentVal + (nextVal - currentVal) * weight;
            result.push({
...
app.post("/api/snapshots", (req, res) => {
  ...
  db.run(
    "INSERT OR REPLACE INTO snapshots (id, name, data) VALUES (?, ?, ?)",
    [id, name, JSON.stringify(record)],
    (err) => {
      if (err) return res.status(500).json({ error: err.message });
