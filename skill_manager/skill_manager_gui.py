...
class SkillManagerGUI:
    ...
    def create_action_buttons(self, parent, row):
        ...
        ttk.Button(btn_frame, text="安装本地Skill", command=self.install_local_skill).pack(
            side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="📥 从来源下载", command=self.download_from_awesome_skills).pack(
            side=tk.LEFT, padx=(0, 5))
    ...
    def download_from_awesome_skills(self):
        """从配置的来源下载skills"""
        ...
        if not sources:
            messagebox.showwarning("提示", "没有配置任何来源,请先添加来源")
            return
        
        # 创建选择对话框
        dialog = tk.Toplevel(self.root)
        dialog.title("从来源下载Skill")
        ...
        # 来源选择
        ttk.Label(main_frame, text="选择来源:", font=('Arial', 11, 'bold')).grid(
            row=0, column=0, sticky=tk.W, pady=(0, 10))
        
        source_var = tk.StringVar()
        source_combo = ttk.Combobox(main_frame, textvariable=source_var, state='readonly', width=50)
        source_combo['values'] = [s['name'] for s in sources]
        if sources:
            source_combo.current(0)
        source_combo.grid(row=0, column=1, sticky=(tk.W, tk.E), pady=(0, 10))
        
        # Skill列表
        ttk.Label(main_frame, text="选择Skill:", font=('Arial', 11, 'bold')).grid(
            row=1, column=0, sticky=tk.W, pady=(10, 5))
        
        skill_listbox = tk.Listbox(main_frame, height=15, width=80)
        skill_listbox.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=(0, 10))
        main_frame.rowconfigure(2, weight=1)
        
        scrollbar = ttk.Scrollbar(main_frame, orient=tk.VERTICAL, command=skill_listbox.yview)
        skill_listbox.configure(yscrollcommand=scrollbar.set)
        ...
        def update_skills(*args):
            ...
            if selected_source and 'skills' in selected_source:
                for skill in selected_source['skills']:
                    skill_listbox.insert(tk.END, f"{skill['name']} - {skill.get('description', '')}")
        
        source_var.trace('w', update_skills)
        update_skills()
        ...
        def download():
            ...
            if not selection:
                messagebox.showwarning("提示", "请先选择要下载的skill")
                return
            ...
            skill_index = selection[0]
            skill = selected_source['skills'][skill_index]
            
            # 下载skill
            self.log_message(f"📥 开始下载: {skill['url']}", 'info')
            
            def progress_callback(msg):
                self.log_message(msg, 'info')
            
            success, message = self.manager.download_skill(
                skill['url'],
                skill['name'],
                progress_callback
            )
        ...
        # 添加浏览仓库功能
        ttk.Label(main_frame, text="或直接输入仓库URL:", font=('Arial', 10, 'bold')).grid(
        ...
        url_entry.insert(0, "https://github.com/user/repo")
        
        def browse_repo():
            """浏览并下载仓库"""
            url = url_entry.get().strip()
            if not url or url == "https://github.com/user/repo":
                messagebox.showwarning("提示", "请输入有效的仓库URL")
                return
            ...
            # 确认下载
            if messagebox.askyesno("确认下载", f"确定要下载仓库 '{repo_name}' 吗?"):
                self.log_message(f"📥 开始下载: {url}", 'info')
        ...
        ttk.Button(url_frame, text="浏览并下载", command=browse_repo).pack(side=tk.LEFT)
        
        btn_frame = ttk.Frame(main_frame)
        btn_frame.grid(row=5, column=0, columnspan=2, pady=(10, 0))
        
        ttk.Button(btn_frame, text="下载选中", command=download).pack(side=tk.LEFT, padx=5)
