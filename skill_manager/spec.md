...
### 8.5 "new-energy-safety-audit" 技能深度优化
- **日期**: 2026-01-29
- **发现问题**:
    - **阈值冲突**: SKILL.md 与提示词中关于 0.5% 和 1% 的误差判定不统一。
    - **数据冗余**: 场站容量和区域信息在多个文件中硬编码，存在同步风险。
    - **管理诊断薄弱**: 缺乏对制度汇编 PDF 的实质性引用。
- **优化方案**:
    - **统一度量衡**: 统一规定电量误差阈值为 0.5%，比率/精度误差为 1%。
    - **数据解耦**: 将区域信息整合进 `stations.md`，移除提示词中的硬编码场站列表。
    - **闭环管理升级**: 强化 AI 对 PDF 制度文档的检索要求，确保整改措施合规。
    - **路径修正**: 更新 `skills_config.json` 指向正确的 `skills/yangjie/` 目录。
### 8.6 "new-energy-safety-audit" 技能新增限电判定逻辑
- **日期**: 2026-01-29
- **背景**: 增加光伏电站限电判断的专业逻辑，辅助审核人员准确识别场站是否处于被动限电还是自身故障。
- **功能**: 
    - 引入四个分析维度：辐照强度、场站可用功率、实际功率、AGC调度远方指令值。
    - 定义三种核心场景：调度限电、未限电、站内异常受阻。
    - 增加 AGC 系统精度导致偏差的特殊案例分析。
- **涉及文件**:
    - `references/curtailment_judgment.md` (新增)
    - `SKILL.md` (更新引用)
    - `GEM.md` (同步更新)
