...
### 6. 2026-02-05 09:48: 完善容量维度 - 增加直流装机容量 ✅

- [x] `spec.md`: 数据模型新增“直流容量”字段。
- [x] `dispatch_stations.md`: 为所有场站补充直流容量 (MWp) 数据。
- [x] `dispatch_stations.md`: 新增宁明、江州、凭祥、扶绥、天等、龙州等 6 个整县分布式项目。
- [x] 逻辑处理：对组合场站（如守旗、岑凡、弄滩）进行容量按比分摊或汇总。
...
### 8. 2026-02-05 20:55: 建立可研项目与风机数据库 ✅

- [x] 提取 `learn.md` 中 15 个项目的 25 年可研利用小时与发电量数据。
- [x] 提取 `learn.md` 中 4 个风电项目（大新龙门、宁明那楠、天等把荷、垌心）的分机位设计参数。
- [x] 建立 `database/feasibility_data.json` 标准化存储可研指标。
- [x] 建立 `database/turbine_design_data.json` 存储风机微观选址与设计参数。
- [x] `spec.md`: 新增可研数据与风机数据模型说明，完善专家库架构。
- [x] 清理任务临时脚本。

### 9. 2026-02-05 21:55: 报价合理性网页翻译 ✅

- [x] 将 `报价合理性/index.html` 报告页面完整翻译为中文。
- [x] 优化图表标签、技术术语（如二次安防、PMU、AGC/AVC 等）的中文表达。
- [x] 更新 `log/debug.log` 记录。
